...
\f\r"'\`<>=]|("|')|))|$)`, "g");
  ...
  async function fetchAssessmentData(assessmentType) {
  ...
  // src/components/field-renderer.ts
  var FieldRenderer = class extends r4 {
    constructor() {
      super(...arguments);
      this.value = "";
      this.rowIndex = 0;
          ...
          return x`
                    <label>
                        ...
                        >
                            ...
                            ${options?.map((option) => x`<option value="${option}">${option}</option>`)}
                        </select>
                    </label>
                `;
        case "boolean":
        ...
        case "number":
          return x`
                    <label>
                        <input
                            type="number"
                            .value="${this.value !== void 0 ? String(this.value) : ""}" // Safeguard against undefined
                            ?required="${required}"
                            @input="${this.handleInputChange}"
                        />
                    </label>
  ...
  __decorateClass([
    n4({ type: Object })
  ], FieldRenderer.prototype, "column", 2);
  __decorateClass([
    n4({ type: String })
  ], FieldRenderer.prototype, "value", 2);
  __decorateClass([
    n4({ type: Number })
  ], FieldRenderer.prototype, "rowIndex", 2);
  FieldRenderer = __decorateClass([
    t3("field-renderer")
  ], FieldRenderer);

  // src/components/dynamic-assessment-form.ts
  var DynamicAssessmentForm = class extends r4 {
    constructor() {
      super(...arguments);
     ...
     * @param event 
     */
    async onAssessmentChange(event) {
      console.log("onAssessmentChange");
      const select = event.target;
      const assessmentType = select.value;
      if (assessmentType) {
        try {
          const assessmentData = await fetchAssessmentData(assessmentType);
      ...
      if (this.validateForm()) {
        try {
          const response = await fetch("http://localhost:3000/api/submit", {
            // Update with your actual submit endpoint
            method: "POST",
    ...
    renderForm() {
      ...
      <form @submit="${this.handleSubmit}">
        <table>
          <thead>
            <tr>
              ${this.metadata ? this.metadata.columns.map((column) => x`<th>${column.label}</th>`) : null}
            </tr>
          </thead>
          <tbody>
        ...
        return x`
          ...
          return x`
                      <td>
                        ...
                        ></field-renderer>
                        ${this.errors[`${column.label}-${rowIndex}`] ? x`
    ...
    render() {
      ...
      <select id="assessmentType" @change="${this.onAssessmentChange}">
        <option value="">Select...</option>
        ${this.assessments.map((type) => x`<option value="${type}">${type}</option>`)}
      </select>
      ${this.renderForm()}
    `;
    }
  };
  DynamicAssessmentForm.styles = i`
  ...
  __decorateClass([
    n4({ type: Array })
  ], DynamicAssessmentForm.prototype, "assessments", 2);
  __decorateClass([
    r6()
  ], DynamicAssessmentForm.prototype, "metadata", 2);
  __decorateClass([
    r6()
  ], DynamicAssessmentForm.prototype, "formData", 2);
  __decorateClass([
    r6()
  ], DynamicAssessmentForm.prototype, "errors", 2);
  DynamicAssessmentForm = __decorateClass([
    t3("dynamic-assessment-form")
