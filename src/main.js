...
(() => {
  ...
  var b = class extends HTMLElement {
    ...
    static getPropertyDescriptor(t4, s3, i5) {
      const { get: e5, set: h3 } = r2(this.prototype, t4) ?? {
        get() {
          return this[s3];
        }, set(t5) {
          this[s3] = t5;
    ...
    renderForm() {
      ...
      <form @submit="${this.handleSubmit}">
        <table>
          <thead>
            <tr>
              ${this.metadata.columns.map((column) => x`<th>${column.label}</th>`)}
            </tr>
          </thead>
          <tbody>
            ${this.formData.map((row, rowIndex) => x`
              <tr>
                ${this.metadata ? this.metadata.columns.map((column) => x`
                  <td>
                    <field-renderer
                      .column="${column}"
